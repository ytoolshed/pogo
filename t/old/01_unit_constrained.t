...
# Test scenario #1: use farm 6195, which has one host of each type, so there
# shouldn't be any blocking (other than sequencing)
...
is( $idx2,  33,    "cached snapshot index" );
is( $snap2, $snap, "cached snapshot data" );

my ( $idx3, $snap3 ) = $job->snapshot(40);
is( $idx3, 33, "partial snapshot index" );
