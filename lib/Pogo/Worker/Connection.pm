  ...
  $self->{dispatcher_handle} = AnyEvent::Handle->new(
    ...
    on_connect => sub {
      ...
      while (my $msg = Pogo::Worker->dequeue_msg) {
        DEBUG sprintf("sending queued response to %s:%d: %s", $self->{host}, $self->{port}, encode_json($msg));
    ...
    on_starttls => sub {
      ...
      my $msg = $_[2];
      if ($success) {
        INFO sprintf("SSL/TLS handshake completed with dispatcher at %s:%d", $self->{host}, $self->{port});
      } else {
        $self->{dispatcher_handle}->destroy;
        ERROR sprintf("Failed to complete SSL/TLS handshake with %s:%d: %s", $self->{host}, $self->{port}, $msg);
        $self->reconnect;
      }
    },
    ...
    on_error => sub {
      ...
      $self->{dispatcher_handle}->destroy;
      Pogo::Worker->delete_connection($self);
      ERROR sprintf( "I/O error occurred while communicating with %s:%d: %s", $self->{host}, $self->{port}, $msg);
      $self->reconnect;
    },
    on_read => sub {
      $self->{dispatcher_handle}->push_read(json => sub {
          my $obj = $_[1];
          if (ref $obj ne 'ARRAY') {
...
sub execute    #{{{
{
  ...
  my $save_dir        = Pogo::Worker->data_dir . '/' . $job_id;
  my $buf_count       = 0;
  my ($output_filename, $output_url);
  my $n = 0;
  do {
    $output_filename = sprintf( "%s/%s/%s.%d.txt", Pogo::Worker->data_dir, $job_id, $host, $n );
  ...
  my $output_file = IO::File->new( $output_filename, O_WRONLY | O_CREAT | O_APPEND, 0664 )
    or LOGDIE "Couldn't create file: $!";
  ...
  my $write_stdout = sub {
    ...
    if (Pogo::Worker->max_output && $buf_count >= Pogo::Worker->max_output)
    {
      ...
      $self->write_output_entry( $output_file, { task => $task_id, type => 'STDOUT' },
...
sub reset #{{{
{
  ...
  my $task = $self->{tasks}->{$task_id};
  Pogo::Worker->send_response( ['finish', $task->{args}->{job_id}, $task->{args}->{host}, $code, $msg] );
...
sub reconnect         #{{{
{
  my ( $self, $interval ) = @_;
  $interval ||= rand(30);
  INFO sprintf( "Reattempting connection to %s:%d in %.2f seconds.", $self->{host}, $self->{port}, $interval );
