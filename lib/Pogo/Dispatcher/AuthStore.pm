...
sub store
{
  ...
  my ( $self, $job, $pw, $secrets, $expire, $pvt_key_passphrase, $client_private_key ) = @_;

  # stash locally
  _store_local( $job, $pw, $secrets, $expire, $pvt_key_passphrase, $client_private_key );

  # store on hosts in the peerlist
  $_->push_write( json => [ 'storesecrets', $job, $pw, $secrets, $expire, $pvt_key_passphrase, $client_private_key ] )
...
sub _store_local
{
  ...
  my ( $job, $pw, $secrets, $expire, $pvt_key_passphrase, $client_private_key ) = @_;

  INFO "stored secrets for job $job";
  $instance->{secrets}->{$job} = [ $pw, $secrets, $expire, $pvt_key_passphrase, $client_private_key ];
        ...
        on_read => sub {
          $handle->push_read(
            json => sub {
              ...
              if ( $cmd eq 'store' )
              {
                my ( $jobid, $pw, $secrets, $expire, $pvt_key_passphrase, $client_private_key ) = @args;
                DEBUG "got secretss for job $jobid from authstore peer $host:$port";
                _store_local( $jobid, $pw, $secrets, $expire, $pvt_key_passphrase, $client_private_key );
              }
              elsif ( $cmd eq 'expire' )
              {
                my ($job) = @args;
